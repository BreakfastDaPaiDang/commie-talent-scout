...
export function ImageGallery({images,onRemove}:{images:AttachmentInfo[];onRemove?:(id:string)=>void}){
 const[selected,setSelected]=useState<AttachmentInfo|null>(null);
 return <>{images.length>0&&<div className="image-gallery">{images.map((i,n)=><div key={i.id}><button type="button" aria-label={`放大图片 ${n+1}`} onClick={()=>setSelected(i)}><img src={'/images/'+i.id} alt={`观察图片 ${n+1}`} loading="lazy" decoding="async" width={i.width} height={i.height}/></button>{onRemove&&<button className="image-remove" type="button" aria-label={`移除图片 ${n+1}`} onClick={()=>onRemove(i.id)}>×</button>}</div>)}</div>}{selected&&<Modal title="查看原图" wide onClose={()=>setSelected(null)}><ImageViewer key={selected.id} src={'/images/'+selected.id} alt="观察原图" width={selected.width} height={selected.height}/></Modal>}</>;
}
