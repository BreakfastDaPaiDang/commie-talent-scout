...
type Draft={attachments:AttachmentInfo[];body:string;occurred_at:string|null;version:number;publish_request_id:string;updated_at:string};
type Snapshot={attachments:AttachmentInfo[];body:string;occurred_at:string|null;version:number;publishId:string|null;publishedId:string|null;loaded:boolean;phase:'loading'|'saved'|'saving'|'unsaved'|'error';error:string;dirty:boolean};
type Save={attachment_ids:string[];archive_id:string;expected_version:number;body:string;occurred_at:string|null;request_id:string};
const stores=new Map<string,DraftStore>();
const changed=()=>window.dispatchEvent(new Event('cts-drafts-changed'));
class DraftStore{
 private state:Snapshot={attachments:[],body:'',occurred_at:null,version:0,publishId:null,publishedId:null,loaded:false,phase:'loading',error:'',dirty:false};
 ...
 load(force=false){
  ...
  this.loading=(async()=>{try{const r=await api<{draft:Draft|null;published:{id:string}|null}>('/drafts/'+this.archiveId);if(this.state.dirty&&!force)return;this.pending=null;this.update({attachments:r.draft?.attachments??[],body:r.draft?.body??'',occurred_at:r.draft?.occurred_at??null,version:r.draft?.version??0,publishId:r.draft?.publish_request_id??null,publishedId:r.published?.id??null,loaded:true,phase:'saved',error:'',dirty:false});}catch(e){this.update({loaded:false,phase:'error',error:(e as Error).message});throw e;}finally{this.loading=undefined;}})();return this.loading;
 }
 edit(patch:Partial<Pick<Snapshot,'body'|'occurred_at'|'attachments'>>){this.update({...patch,dirty:true,phase:'unsaved',error:'',publishedId:null});clearTimeout(this.timer);this.timer=setTimeout(()=>void this.flush().catch(()=>{}),650);}
 flush():Promise<void>{
  ...
  this.running=(async()=>{try{
   while(this.state.dirty||this.pending){
    const payload=this.pending??{attachment_ids:this.state.attachments.map(i=>i.id),archive_id:this.archiveId,expected_version:this.state.version,body:this.state.body,occurred_at:this.state.occurred_at,request_id:crypto.randomUUID()};this.pending=payload;this.update({phase:'saving',error:''});
    const result=await api<{version:number;publish_request_id:string}>('/drafts/save',payload);this.pending=null;
    const dirty=JSON.stringify(this.state.attachments.map(i=>i.id))!==JSON.stringify(payload.attachment_ids)||this.state.body!==payload.body||this.state.occurred_at!==payload.occurred_at;this.update({version:result.version,publishId:result.publish_request_id,dirty,phase:dirty?'unsaved':'saved'});
   }
 ...
 async clearPublished(){this.edit({body:'',occurred_at:null,attachments:[]});await this.flush();}
}
