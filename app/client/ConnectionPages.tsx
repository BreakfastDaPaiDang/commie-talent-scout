...
export function AgentPage(){
  ...
  return <main className="support-content"><section className="agent-simple"><div className="agent-copy"><span className="eyebrow">让材料成为观察</span><h1>复制，交给你的 Agent。</h1><p>把提示词粘贴到你常用的 Agent，<br/>由它带你完成接入。</p><p className="agent-examples">调用及提交内容用于排错和改进。<br/>正文保留 30 天，调用信息保留 180 天。</p><button className="button primary" onClick={copy}>复制提示词给 Agent</button>{copied&&<p role="status">提示词已复制，粘贴给 Agent 即可。</p>}{fallback&&<div className="copy-fallback"><label>请选择下方正文复制<textarea readOnly value={prompt} onFocus={e=>e.currentTarget.select()} rows={12}/></label></div>}</div><div className="agent-art" aria-hidden="true"><img src="/art/agent-handoff-v3.png" alt=""/></div></section></main>;
}
...
type Call={id:string;member_id:string;credential_id:string;member_name:string|null;connection_name:string|null;tool:string;started_at:string;duration_ms:number|null;outcome:string;error_code:string|null};
function CallDetails({id}:{id:string}){
  ...
  return <details className="call-details" onToggle={e=>{if(e.currentTarget.open&&!data)api<{parameters:unknown;result:unknown;body_state:string}>('/admin/calls/'+id).then(setData).catch(e=>setError(e.message));}}><summary>查看请求与结果</summary>{error?<p role="alert">{error}</p>:!data?<p role="status">正在读取…</p>:<><p>{data.body_state==='expired'?'正文已到期清理，保留调用信息。':data.body_state==='retained'?'已筛选的提交内容与执行摘要':'正文未完整留存'}</p>{data.parameters!==null&&<><h3>提交内容</h3><pre>{JSON.stringify(data.parameters,null,2)}</pre></>}{data.result!==null&&<><h3>执行摘要</h3><pre>{JSON.stringify(data.result,null,2)}</pre></>}</>}</details>;
}
const outcomes:Record<string,string>={success:'成功',no_change:'无变化',rejected:'拒绝',failed:'失败',unknown:'结果不明'};
export function CallsPage(){
  const[items,setItems]=useState<Call[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[next,setNext]=useState<string|null>(null),[outcome,setOutcome]=useState(''),[from,setFrom]=useState(''),[to,setTo]=useState(''),[coverage,setCoverage]=useState(''),[diagnostics,setDiagnostics]=useState<{day:string;kind:string;count:number}[]>([]);
  async function load(cursor?:string){setLoading(true);setError('');const query=new URLSearchParams();if(outcome)query.set('outcome',outcome);if(from)query.set('from',new Date(from).toISOString());if(to)query.set('to',new Date(to).toISOString());if(cursor)query.set('before',cursor);try{const r=await api<{calls:Call[];next_cursor:string|null;coverage:string;diagnostics:{day:string;kind:string;count:number}[]}>('/admin/calls?'+query);setItems(old=>cursor?[...old,...r.calls]:r.calls);setNext(r.next_cursor);setCoverage(r.coverage);setDiagnostics(r.diagnostics);}catch(e){setError((e as Error).message);}finally{setLoading(false);}}
  useEffect(()=>{void load();},[]);
  return <main className="account-content"><div className="section-title"><p className="eyebrow">管理员</p><h1>Agent 调用</h1><p>查看到达服务的调用和实际结果。</p></div><form className="call-filters" onSubmit={e=>{e.preventDefault();void load();}}><label>结果<select value={outcome} onChange={e=>setOutcome(e.target.value)}><option value="">全部结果</option>{Object.entries(outcomes).map(([k,v])=><option value={k} key={k}>{v}</option>)}</select></label><label>开始时间<input type="datetime-local" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>结束时间<input type="datetime-local" value={to} onChange={e=>setTo(e.target.value)}/></label><button className="button" disabled={loading}>筛选</button></form><PageError error={error} retry={()=>void load()}/><p className="secondary-copy">{coverage}</p>{diagnostics.length>0&&<details className="diagnostic-note"><summary>采集与认证诊断（最近 30 天）</summary>{diagnostics.map(x=><p key={x.day+x.kind}>{x.day} · {x.kind} · {x.count} 次</p>)}</details>}
    <ul className="call-list">{items.map(item=><li key={item.id}><div className="call-heading"><strong>{item.tool}</strong><span className={'outcome '+item.outcome}>{outcomes[item.outcome]}</span></div><p>{new Date(item.started_at).toLocaleString('zh-CN')} · {item.duration_ms??'—'} ms</p><small>{item.member_name??'历史成员'} · {item.connection_name??'已失效连接'}</small>{item.error_code&&<p className="danger-text">{item.error_code}</p>}<CallDetails id={item.id}/></li>)}</ul>{loading?<p role="status">正在读取调用…</p>:items.length===0?<p className="empty-state">这个范围内没有调用记录。</p>:next&&<button className="button" onClick={()=>void load(next)}>加载更多</button>}
  </main>;
}
