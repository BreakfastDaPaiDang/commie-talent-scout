...
export function CallsPage(){
  ...
  return <main className="account-content"><div className="section-title"><p className="eyebrow">管理员</p><h1>Agent 调用</h1><p>查看到达服务的调用和实际结果。</p></div><form className="call-filters" onSubmit={e=>{e.preventDefault();void load();}}><label>结果<select value={outcome} onChange={e=>setOutcome(e.target.value)}><option value="">全部结果</option>{Object.entries(outcomes).map(([k,v])=><option value={k} key={k}>{v}</option>)}</select></label><label>开始时间<input type="datetime-local" value={from} onChange={e=>setFrom(e.target.value)}/></label><label>结束时间<input type="datetime-local" value={to} onChange={e=>setTo(e.target.value)}/></label><button className="button" disabled={loading}>筛选</button></form><PageError error={error} retry={()=>void load()}/><p className="secondary-copy">{coverage}</p>{diagnostics.length>0&&<details className="diagnostic-note"><summary>采集与认证诊断（最近 30 天）</summary>{diagnostics.map(x=><p key={x.day+x.kind}>{x.day} · {x.kind} · {x.count} 次</p>)}</details>}
    <ul className="call-list">{items.map(item=><li key={item.id}><div className="call-heading"><strong>{item.tool}</strong><span className={'outcome '+item.outcome}>{outcomes[item.outcome]}</span></div><p>{new Date(item.started_at).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',year:'numeric',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit',hour12:false})} · {item.duration_ms??'—'} ms</p><small>{item.member_name??'历史成员'} · {item.connection_name??'已失效连接'}</small>{item.error_code&&<p className="danger-text">{item.error_code}</p>}<CallDetails id={item.id}/></li>)}</ul>{loading?<p role="status">正在读取调用…</p>:items.length===0?<p className="empty-state">这个范围内没有调用记录。</p>:next&&<button className="button" onClick={()=>void load(next)}>加载更多</button>}
  </main>;
}
