...
function App(){
  ...
  return <div className="app"><header className="topbar"><Brand/><nav className="primary-nav" aria-label="主要导航"><button className={path==='/'?'active':''} onClick={()=>navigate('/')}>人物</button><button className={path==='/organizations'?'active':''} onClick={()=>navigate('/organizations')}>组织</button><button className={path==='/unread'?'active':''} onClick={()=>navigate('/unread')}>未读<UnreadBadge key={member.id}/></button><button className={path==='/tags'?'active':''} onClick={()=>navigate('/tags')}>标签库</button><button className={path==='/agent'?'active':''} onClick={()=>navigate('/agent')}>Agent 接入</button></nav><div className="topbar-tools"><details className="account-menu"><summary>{member.name}<span aria-hidden="true"> ▾</span></summary><div><button onClick={()=>navigate('/account/profile')}>我的资料</button><button onClick={()=>navigate('/account/connections')}>我的连接</button><button onClick={()=>navigate('/account/password')}>修改密码</button>{member.role==='admin'&&<><button onClick={()=>navigate('/admin/members')}>猎头管理</button><button onClick={()=>navigate('/admin/calls')}>Agent 调用</button></>}<button disabled={busy} onClick={logout}>退出登录</button></div></details></div></header>{notice&&<p className="site-notice" role="status">{notice}</p>}{error&&<p className="form-error" role="alert">{error}</p>}
