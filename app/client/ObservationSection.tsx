...
function EventCard({event:e,expanded=false}:{event:TimelineEvent;expanded?:boolean}){
 ...
 return <article className="compact-event" id={'event-'+e.id}><span aria-hidden="true">◇</span><div><p><strong>{kinds[e.kind]??e.kind}</strong><small>{e.actor_name} · {time(e.created_at)} · {e.source==='mcp'?'MCP':'网页'}</small></p>{e.observation_id&&<p>{e.kind==='observation.deleted'?'这条观察已删除；这里只显示删除动态。':'这是该观察的历史动态，正文在最新动态或观察列表中查看。'}</p>}{hasChange?<details open={expanded||!!e.observation_id||undefined}><summary>查看{e.before?'变化':'内容'}</summary><div className="reading-content"><ReadBoundary reading={e.reading}/>{change}</div></details>:<div className="reading-content"><ReadBoundary reading={e.reading}/><span>该动态没有其他内容。</span></div>}</div></article>;
}
