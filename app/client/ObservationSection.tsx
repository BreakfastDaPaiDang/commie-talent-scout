...
function ObservationComposer({actor,archive,onPublished}:{actor:Member;archive:Archive;onPublished:()=>void}){
 ...
 return <form className="live-composer-form" onSubmit={publish} onPaste={e=>{if(!busy&&draft.loaded)setError(pasteImages(e,actor.id+':draft:'+archive.id,{purpose:'observation',archive_id:archive.id},draft.attachments.length));}}><ComposerFrame open={open} onOpen={()=>setOpen(true)} textarea={{value:draft.body,onChange:e=>store.edit({body:e.target.value}),maxLength:100000,disabled:busy||!draft.loaded}}>
  <ImagePicker queueKey={actor.id+':draft:'+archive.id} target={{purpose:'observation',archive_id:archive.id}} value={draft.attachments} onChange={attachments=>store.edit({attachments})} onPending={setPending} disabled={busy||!draft.loaded}/>
  <div className="composer-foot"><details className="occurred-control"><summary>补录发生时间</summary><label>观察发生时间<input type="datetime-local" value={localTime(draft.occurred_at)} disabled={busy||!draft.loaded} onChange={e=>store.edit({occurred_at:isoTime(e.target.value)})}/></label></details><button className="button quiet" type="button" onClick={()=>setOpen(false)}>收起</button><button className="button primary" disabled={busy||pending||!draft.loaded||(!draft.body.trim()&&!draft.attachments.length)||!!draft.publishedId}>{busy?'正在发布…':'发布记录'}</button></div>
