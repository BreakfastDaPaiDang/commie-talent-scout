...
export function ObservationSection({actor,archive,onChanged}:{actor:Member;archive:Archive;onChanged:()=>void}){
 ...
 function renderRecord(record:Observation){return <ObservationCard key={record.id} record={record} onEdit={()=>setEdit(record)} onHistory={()=>setHistory(record)} onState={()=>setStateTarget(record)}/>;}
...
function ObservationCard({record,onEdit,onHistory,onState}:{record:Observation;onEdit:()=>void;onHistory:()=>void;onState:()=>void}){
 return <article className="observation-card" id={'observation-'+record.id}><header><Avatar name={record.author_name} src={record.author_avatar_url} size="tiny"/><div><strong>{record.author_name}</strong><p>提交于 {time(record.created_at)}{record.content_version>1&&<span> · 已编辑 {time(record.updated_at)}</span>}</p></div><div className="observation-actions">{record.editable&&<button className="button quiet" onClick={onEdit}>编辑</button>}<button className="button quiet" onClick={onHistory}>历史</button>{(record.deletable||record.restorable)&&<button className="button quiet" onClick={onState}>{record.deleted?'恢复':'删除'}</button>}</div></header>{record.deleted&&<p className="form-notice">已由 {record.deleted_by_name??'成员'} 于 {record.deleted_at?time(record.deleted_at):'未知时间'} 删除。正文与图片仅原作者及管理员可见。{!record.restorable&&'档案已关闭，暂不可恢复。'}</p>}{record.occurred_at&&<p className="observation-occurred">观察发生于 {time(record.occurred_at)}</p>}<div className="observation-body">{record.body}</div><ImageGallery images={record.attachments}/></article>;
}
function ObservationComposer({actor,archive,onPublished}:{actor:Member;archive:Archive;onPublished:()=>void}){
 ...
 return <section className={'observation-composer '+(open?'expanded':'')}>
  {!open?<button className="start-observation" onClick={()=>setOpen(true)}><span aria-hidden="true">＋</span> 写一条观察<span>留下一件具体的事</span></button>:<form onSubmit={publish} onPaste={e=>{if(!busy&&draft.loaded)setError(pasteImages(e,actor.id+':draft:'+archive.id,{purpose:'observation',archive_id:archive.id},draft.attachments.length));}}><div className="composer-heading"><h2>新的观察</h2><button className="button quiet" type="button" onClick={()=>setOpen(false)} disabled={busy||pending}>收起</button></div><label className="sr-only" htmlFor={'observation-body-'+archive.id}>观察正文</label><textarea id={'observation-body-'+archive.id} value={draft.body} maxLength={100000} rows={7} placeholder="发生了什么？你从中观察到了什么？" onChange={e=>store.edit({body:e.target.value})} disabled={busy||!draft.loaded}/><ImagePicker queueKey={actor.id+':draft:'+archive.id} target={{purpose:'observation',archive_id:archive.id}} value={draft.attachments} onChange={attachments=>store.edit({attachments})} onPending={setPending} disabled={busy||!draft.loaded}/><div className="composer-meta"><label>观察发生时间 <small>可选，补录时填写</small><input type="datetime-local" value={localTime(draft.occurred_at)} disabled={busy||!draft.loaded} onChange={e=>store.edit({occurred_at:isoTime(e.target.value)})}/></label><button className="button primary" disabled={busy||pending||!draft.loaded||(!draft.body.trim()&&!draft.attachments.length)||!!draft.publishedId}>{busy?'正在发布…':'发布观察'}</button></div>
...
function ObservationEdit({actorId,record,busy,setBusy,onSaved,onReload}:{actorId:string;record:Observation;busy:boolean;setBusy:(b:boolean)=>void;onSaved:()=>Promise<void>;onReload:()=>Promise<void>}){
 const[attachments,setAttachments]=useState(record.attachments),[pending,setPending]=useState(false),[body,setBody]=useState(record.body),[occurred,setOccurred]=useState(record.occurred_at),[requestId]=useState(()=>crypto.randomUUID()),[error,setError]=useState('');
 async function save(e:FormEvent){e.preventDefault();setBusy(true);setError('');try{await api('/observations/update',{id:record.id,expected_version:record.version,body,attachment_ids:attachments.map(i=>i.id),occurred_at:occurred,request_id:requestId});await onSaved();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 return <form className="manage-form observation-edit-form" onSubmit={save} onPaste={e=>{if(!busy)setError(pasteImages(e,actorId+':edit:'+record.id,{purpose:'observation',archive_id:record.archive_id},attachments.length));}}><p>原作者：{record.author_name}。保存后保留当前版本。</p><label>观察正文<textarea value={body} onChange={e=>setBody(e.target.value)} maxLength={100000} rows={13} disabled={busy}/></label><ImagePicker queueKey={actorId+':edit:'+record.id} target={{purpose:'observation',archive_id:record.archive_id}} value={attachments} onChange={setAttachments} onPending={setPending} disabled={busy}/><label>观察发生时间<input type="datetime-local" value={localTime(occurred)} onChange={e=>setOccurred(isoTime(e.target.value))} disabled={busy}/></label><PageError error={error} retry={()=>void onReload().catch(e=>setError(e.message))} retryLabel="重新读取并替换表单"/><div className="dialog-actions"><button className="button primary" disabled={busy||pending||(!body.trim()&&!attachments.length)}>{busy?'正在保存…':'保存新版本'}</button></div></form>;
}
