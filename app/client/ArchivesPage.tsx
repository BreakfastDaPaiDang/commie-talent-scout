...
export function ArchivesPage({actor,type}:{actor:Member;type:Kind}){
 ...
 const[dialog,setDialog]=useState<'create'|Archive|null>(null),[busy,setBusy]=useState(false),[selected,setSelected]=useState<Archive|null>(null);
 ...
 async function saved(id:string,changed:boolean){setDialog(null);choose(id);setRevision(v=>v+1);setNotice(changed?'档案已保存':'资料没有变化');}
 return <main className={'archives-workspace '+(pref.selected?'has-selection':'')}>
  <section className="archives-list-panel" aria-label={`${label}档案列表`}>
   ...
   <div className="archives-rows">{items.map(a=><button key={a.id} className={'archive-row '+(pref.selected===a.id?'selected':'')} onClick={()=>choose(a.id)} aria-label={`查看${a.name}`}><Avatar name={a.name} type={a.type}/><div className="archive-row-content"><div className="archive-row-title"><h2>{a.name}</h2></div><div className="archive-current"><span className={'state-badge small '+(a.closed?'closed':'')}>{a.status}</span>{a.members.length>0&&<span className="archive-assignees">关联 · {a.members.map(m=>m.name+(m.frozen?'（冻结）':'')).join('、')}</span>}</div><p className="archive-row-excerpt">还没有观察记录</p><div className="archive-row-footer"><span>{a.observation_count} 条观察</span><time>{date(a.updated_at)}</time></div></div></button>)}</div>
  ...
  <section className="archive-detail-panel" aria-label="档案详情">
   {pref.selected?<ArchiveDetail key={pref.selected} id={pref.selected} revision={revision} onLoaded={setSelected} onBack={()=>choose(null)} onEdit={()=>selected&&setDialog(selected)} notice={notice}/>:<div className="archive-welcome"><p className="eyebrow">持续观察 · 共同沉淀</p><Avatar type={type} size="large"/><h2>留意那些<br/>值得了解的{label==='人物'?'人':'伙伴'}。</h2><p>从左侧打开一份档案，或创建新的关注对象。</p></div>}
  </section>
...
function ArchiveDetail({id,revision,onLoaded,onBack,onEdit,notice}:{id:string;revision:number;onLoaded:(a:Archive)=>void;onBack:()=>void;onEdit:()=>void;notice:string}){
 ...
 return <><div className="archive-detail-top"><button className="button quiet archive-back" onClick={onBack}>← 返回列表</button><span className="eyebrow">观察档案</span>{archive&&!archive.closed&&<button className="button" onClick={onEdit}>编辑资料</button>}</div><PageError error={error} retry={()=>setRetry(n=>n+1)}/>{!archive&&!error?<p className="archive-list-message" role="status">正在打开档案…</p>:archive&&<>
  <div className="archive-profile"><div className="archive-identity"><Avatar name={archive.name} type={archive.type} size="large"/><div><p className="eyebrow">{archive.type==='person'?'人物':'组织'} / {archive.closed?'已关闭':'持续观察'}</p><h1>{archive.name}</h1><span className={'state-badge '+(archive.closed?'closed':'')}>{archive.status}</span></div></div>
   {archive.closed&&<p className="archive-readonly">档案已关闭，当前内容只读。</p>}
...
const eventKinds:Record<string,string>={'archive.created':'创建档案','archive.profile_changed':'修改基础资料'};
function eventValue(value:unknown):string{if(value===undefined||value===null||value==='')return '未填写';if(Array.isArray(value))return value.length?value.map(v=>typeof v==='object'?('url'in v?`${v.label||'链接'} ${v.url}`:`${v.type} ${v.value}${v.note?'（'+v.note+'）':''}`):String(v)).join('；'):'未填写';return String(value);}
function ArchiveEvents({id,revision}:{id:string;revision:number}){
 ...
 const fields:Record<string,string>={name:'名称',contacts:'联系方式',links:'资料链接',status:'业务状态',type:'档案类型'};
...
function ArchiveForm({type,initial,busy,setBusy,onSaved,onReload}:{type:Kind;initial?:Archive;busy:boolean;setBusy:(b:boolean)=>void;onSaved:(id:string,changed:boolean)=>Promise<void>;onReload:()=>Promise<void>}){
 const[name,setName]=useState(initial?.name??''),[contacts,setContacts]=useState(initial?.contacts??[]),[links,setLinks]=useState(initial?.links??[]),[error,setError]=useState(''),[requestId]=useState(()=>crypto.randomUUID()),[similar,setSimilar]=useState<Archive[]>([]);
 ...
 async function save(e:FormEvent){e.preventDefault();setBusy(true);setError('');try{const r=await api<{id:string;changed:boolean}>(initial?'/archives/update':'/archives/create',{...(initial?{id:initial.id,expected_version:initial.version}:{type}),name,contacts,links,request_id:requestId});await onSaved(r.id,r.changed);}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
 return <form className="manage-form archive-form" onSubmit={save}>
  ...
  <PageError error={error} retry={initial?()=>void onReload().catch(e=>setError(e.message)):undefined} retryLabel="重新读取并替换表单"/><div className="dialog-actions"><button className="button primary" disabled={busy}>{busy?'正在保存…':initial?'保存资料':'创建档案'}</button></div>
 </form>;
}
