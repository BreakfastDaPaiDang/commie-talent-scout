import {ArchiveDeletionConfirmation,ArchiveLifecycleNotice} from '../ui/ArchiveLifecycle';
...
export function ArchivesPage({actor,type,linked,detailOnly=false,onNextUnread,onClose,trash=false,onTypeChange}:{trash?:boolean;onTypeChange?:(type:Kind)=>void;actor:Member;type:Kind;linked?:{archiveId:string;focus:ObservationFocus};detailOnly?:boolean;onNextUnread?:()=>void;onClose?:()=>void}){
 ...
 async function load(before?:string){
  ...
  try{const r=await api<{archives:ListArchive[];next_cursor:string|null;counts?:Record<string,number>}>('/archives?'+new URLSearchParams({type,deleted:String(trash),query:pref.query,scope:trash?'all':pref.scope,status:pref.status,member_id:pref.member_id,closed:pref.closed,tag_ids:pref.tag_ids.join(','),...(before?{before}:{})}));if(sequence!==request.current)return;setItems(old=>before?[...old,...r.archives]:r.archives);setCursor(r.next_cursor);if(r.counts)setCounts(r.counts);}catch(e){if(sequence===request.current)setError((e as Error).message);}finally{if(sequence===request.current)setLoading(false);}
 }
 ...
 return <><WorkspaceFrame selected={!!pref.selected} expanded={expanded} detailOnly={detailOnly} list={<>
  ...
  <PageError error={error} retry={()=>void load()}/>{notice&&!pref.selected&&<p className="form-notice archive-list-message" role="status">{notice}</p>}
  ...
  {loading?<p className="archive-list-message" role="status">正在读取档案…</p>:error?null:!items.length?<div className="archive-list-message"><h2>{query||activeFilters||pref.scope!=='all'?'没有符合条件的档案':trash?'没有已删除档案':'从第一个档案开始'}</h2><p>{query?'试试其他关键词，或清除筛选。':trash?'删除的档案会出现在这里。':`把值得关注的${label}留在这里，慢慢积累观察。`}</p></div>:cursor&&<button className="button archive-load" onClick={()=>void load(cursor)}>加载更多档案</button>}
 </>} detail={pref.selected?<ArchiveDetail onDelete={()=>selected&&setDeletion(selected)} type={type} onNextUnread={onNextUnread} expanded={expanded} onExpand={detailOnly?undefined:()=>setExpanded(v=>!v)} focus={focus} actor={actor} onChanged={()=>setRevision(n=>n+1)} key={pref.selected} id={pref.selected} revision={revision} onLoaded={setSelected} onBack={onClose??(detailOnly?undefined:()=>choose(null))} onEdit={()=>{if(selected){setEditTab('profile');setDialog(selected);}}} onState={()=>selected&&setStateDialog(selected)} notice={notice}/>:<aside className="selection-rest" aria-hidden="true"><div className="rest-characters"><Avatar type={type} size="large"/></div><span>打开一个档案，继续观察。</span></aside>}/>
  ...
  {dialog&&<Modal title={dialog==='create'?`新建${label}档案`:'编辑档案'} busy={busy} onClose={()=>setDialog(null)}>
   ...
   {dialog!=='create'&&actor.role==='admin'&&<div className="archive-delete-entry"><button className="text-button" disabled={busy} onClick={()=>{setDeletion(dialog);setDialog(null);}}>删除档案</button></div>}
  </Modal>}
  {deletion&&<Modal title={deletion.deleted?'恢复档案':'删除档案'} busy={busy} onClose={()=>setDeletion(null)}><ArchiveDeletionDialog key={deletion.id+deletion.version} archive={deletion} busy={busy} setBusy={setBusy} onCancel={()=>setDeletion(null)} onReload={async()=>{const r=await api<{archive:Archive}>('/archives/'+deletion.id);setDeletion(r.archive);setRevision(n=>n+1);}} onSaved={()=>{const restored=deletion.deleted;setDeletion(null);setDialog(null);choose(null);setRevision(n=>n+1);setNotice(restored?'档案已恢复':'档案已删除，可在已删除档案中恢复');window.dispatchEvent(new Event('cts-reading-stale'));}}/></Modal>}
...
function ArchiveDeletionDialog({archive,busy,setBusy,onCancel,onSaved,onReload}:{archive:Archive;busy:boolean;setBusy:(v:boolean)=>void;onCancel:()=>void;onSaved:()=>void;onReload:()=>Promise<void>}){
 ...
 async function submit(){setBusy(true);setError('');try{await api(archive.deleted?'/archives/restore':'/archives/delete',{id:archive.id,expected_version:archive.version,request_id:requestId});onSaved();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
