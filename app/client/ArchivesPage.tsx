...
function ArchiveDetail({actor,onChanged,id,revision,onLoaded,onBack,onEdit,onState,notice}:{actor:Member;onChanged:()=>void;id:string;revision:number;onLoaded:(a:Archive)=>void;onBack:()=>void;onEdit:()=>void;onState:()=>void;notice:string}){
 ...
 return <><div className="archive-detail-top"><button className="button quiet archive-back" onClick={onBack}>← 返回列表</button><span className="eyebrow">观察档案</span>{archive&&!archive.closed&&<button className="button" onClick={onEdit}>编辑资料</button>}</div><PageError error={error} retry={()=>setRetry(n=>n+1)}/>{!archive&&!error?<p className="archive-list-message" role="status">正在打开档案…</p>:archive&&<>
  <div className="archive-profile"><div className="archive-identity">{archive.closed?<Avatar name={archive.name} type={archive.type} size="large" src={avatarUrl('archive',archive.id,archive.version)}/>:<AvatarEditor actorId={actor.id} subjectType="archive" id={archive.id} version={archive.version} name={archive.name} kind={archive.type} avatarId={archive.avatar_id} onSaved={onChanged}/>}<div><p className="eyebrow">{archive.type==='person'?'人物':'组织'} / {archive.closed?'已关闭':'持续观察'}</p><h1>{archive.name}</h1><span className={'state-badge '+(archive.closed?'closed':'')}>{archive.status}</span></div></div>
