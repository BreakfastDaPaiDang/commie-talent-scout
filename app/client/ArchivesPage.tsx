...
export function ArchivesPage({actor,type,linked,detailOnly=false,onNextUnread,onClose,trash=false,onTypeChange}:{trash?:boolean;onTypeChange?:(type:Kind)=>void;actor:Member;type:Kind;linked?:{archiveId:string;focus:ObservationFocus};detailOnly?:boolean;onNextUnread?:()=>void;onClose?:()=>void}){
 ...
 async function load(before?:string,refreshLoaded=false){
  const sequence=++request.current,target=refreshLoaded?Math.max(30,items.length):30;setLoading(true);setError('');
  try{
  ...
  }catch(e){if(sequence===request.current)setError((e as Error).message);}finally{if(sequence===request.current)setLoading(false);}
 }
