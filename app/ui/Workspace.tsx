...
export function ScopeToolbar({scope,onScope,counts,filtersOpen,onFilters,activeFilters,filters,caption,onReset}:{scope:string;onScope:(value:string)=>void;counts?:Record<string,number>;filtersOpen:boolean;onFilters:()=>void;activeFilters:boolean;filters:ReactNode;caption:ReactNode;onReset?:()=>void}){
 return <section className="list-toolbar"><div className="scope-filter-row"><div className="scope-tabs" aria-label="档案范围">{[['all','全部'],['mine','我负责'],['unread','有未读']].map(([key,label])=><button key={key} aria-pressed={scope===key} className={scope===key?'active':''} onClick={()=>onScope(key)}>{label}{counts&&<small>{counts[key]}</small>}</button>)}</div><button className={`filter-toggle ${activeFilters?'has-filters':''}`} aria-expanded={filtersOpen} onClick={onFilters}><Icon name="filter" size={18}/>筛选{activeFilters&&<span className="unread-dot"/>}</button></div>{filtersOpen&&<div className="filter-row">{filters}</div>}<div className="list-caption"><span>{caption}</span>{onReset?<button onClick={onReset}>清除条件</button>:<span>最近更新 ↓</span>}</div></section>;
}
export function ArchiveRow({avatar,name,state,binding,excerpt,footer,tags,working,draft,unread,selected,onClick,label}:{avatar:ReactNode;name:ReactNode;state:ReactNode;binding:ReactNode;excerpt:ReactNode;footer:ReactNode;tags?:ReactNode;working?:boolean;draft?:boolean;unread?:boolean;selected:boolean;onClick:()=>void;label?:string}){
 return <button className={`entity-row ${selected?'selected':''}`} onClick={onClick} aria-label={label}>{avatar}<div className="row-content"><div className="row-title"><h2>{name}</h2>{unread&&<span className="unread-dot" aria-label="有未读更新"/>}<Icon name="arrow" size={18}/></div><div className="row-meta">{state}{working&&<span className="work-marker" title="工作状态，须指定负责成员">◆</span>}{draft&&<span className="draft-tag">草稿</span>}</div><div className="row-binding"><span>{working?'负责人':'关联'}</span><span>{binding}</span></div>{tags}<p className="record-excerpt">{excerpt}</p><div className="row-foot">{footer}</div></div></button>;
}
