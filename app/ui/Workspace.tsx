...
export function ArchiveRow({avatar,name,state,status,binding,excerpt,footer,tags,draft,unread,selected,onClick,label}:{avatar:ReactNode;name:ReactNode;state:ReactNode;status?:string;binding:ReactNode;excerpt:ReactNode;footer:ReactNode;tags?:ReactNode;draft?:boolean;unread?:boolean;selected:boolean;onClick:()=>void;label?:string}){
 ...
 return <button className={`entity-row ${selected?'selected':''} ${completed?'completed':''}`} onClick={onClick} aria-label={label}>{avatar}<div className="row-content"><div className="row-heading"><div className="row-title"><h2>{name}</h2>{unread&&<span className="unread-dot" aria-label="有未读更新"/>}</div><div className="row-meta">{state}{draft&&<span className="draft-tag">草稿</span>}</div>{binding&&<div className="row-binding">{binding}</div>}</div>{tags}<p className="record-excerpt">{excerpt}</p><div className="row-foot">{footer}</div></div><CompletionStamp status={status}/></button>;
}
