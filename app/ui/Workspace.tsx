...
export function ArchiveHead({label,count,query,onQuery,onCreate,searchRef}:{label:string;count:string;query:string;onQuery:(value:string)=>void;onCreate?:()=>void;searchRef?:RefObject<HTMLInputElement|null>}){
 ...
 return <header className="page-head archive-head"><div><h1>{label}<span className="page-count">{count}</span></h1></div><div className="search-field"><Icon name="search"/><input ref={inputRef} aria-label="搜索档案" placeholder="搜索档案、观察" value={query} maxLength={200} onChange={e=>onQuery(e.target.value)}/>{query?<IconButton name="close" label="清空搜索" onClick={()=>onQuery('')}/>:<kbd>/</kbd>}</div>{onCreate&&<button className="button primary" onClick={onCreate}><Icon name="plus"/>新建</button>}</header>;
}
export function ScopeToolbar({scope,onScope,counts,filtersOpen,onFilters,activeFilters,filters,onReset,scopeItems=[['all','全部'],['mine','关联我'],['unread','有未读']]}:{scopeItems?:string[][];scope:string;onScope:(value:string)=>void;counts?:Record<string,number>;filtersOpen:boolean;onFilters:()=>void;activeFilters:boolean;filters:ReactNode;onReset?:()=>void}){
 return <section className="list-toolbar"><div className="scope-filter-row"><div className="scope-tabs" aria-label="档案范围">{scopeItems.map(([key,label])=><button key={key} aria-pressed={scope===key} className={scope===key?'active':''} onClick={()=>onScope(key)}>{label}{counts&&<small>{counts[key]}</small>}</button>)}</div><button className={`filter-toggle ${activeFilters?'has-filters':''}`} aria-expanded={filtersOpen} onClick={onFilters}><Icon name="filter" size={18}/>筛选{activeFilters&&<span className="unread-dot"/>}</button></div>{filtersOpen&&<div className="filter-row">{filters}{onReset&&<button className="text-button" onClick={onReset}>清除条件</button>}</div>}</section>;
}
...
export function ArchiveRow({avatar,name,state,status,binding,excerpt,footer,tags,draft,unread,updateReminder,selected,onClick,label}:{avatar:ReactNode;name:ReactNode;state:ReactNode;status?:string;binding:ReactNode;excerpt:ReactNode;footer:ReactNode;tags?:ReactNode;draft?:boolean;unread?:boolean;updateReminder?:ArchiveReminder;selected:boolean;onClick:()=>void;label?:string}){
 const completed=status==='已入伙'||status==='已弃用',overdue=!!updateReminder?.overdue&&!completed;
 return <button className={`entity-row ${selected?'selected':''} ${completed?'completed':''} ${unread?'has-updates':''} ${overdue?'is-overdue':''}`} onClick={onClick} aria-label={label}>{avatar}<div className="row-content">{overdue&&<span className="row-reminder"><span><Icon name="warning" size={16}/><strong>需更新或关闭</strong></span><small>超过 {updateReminder!.threshold_days} 天未更新</small></span>}<div className="row-heading"><div className="row-title"><h2>{name}</h2></div><div className="row-meta">{state}{draft&&<span className="draft-tag">草稿</span>}</div>{binding&&<div className="row-binding">{binding}</div>}</div>{tags}<p className="record-excerpt">{excerpt}</p><div className="row-foot">{footer}</div></div><CompletionStamp status={status}/>{unread&&<span className="row-unread">有更新</span>}</button>;
}
...
export function TimelineTabs({id,value,onChange,deletedCount,materialCount}:{id:string;value:string;onChange:(value:string)=>void;deletedCount?:number;materialCount?:number}){
 return <TabList id={id} value={value} onChange={onChange} label="动态筛选" items={[{key:'all',label:'全部动态'},{key:'observations',label:'观察记录'},{key:'deleted',label:'已删除',count:deletedCount},...(materialCount!==undefined?[{key:'materials',label:'绑定材料',count:materialCount}]:[])]}/>;
}
...
export function RecordBody({body,boundary,renderText,className=''}:{body:string;boundary?:ReactNode;renderText?:(line:string)=>ReactNode;className?:string}){
 const text=(line:string)=>renderText?renderText(line):line;
 const linked=(line:string)=>line.split(/(https?:\/\/[^\s<>]+)/g).map((part,i)=>{try{const url=new URL(part);if(typeof location!=='undefined'&&url.origin===location.origin&&url.searchParams.has('archive')&&url.searchParams.has('material'))return <a key={i} href={url.href} target="_blank" rel="noopener noreferrer">查看绑定材料</a>;}catch{}return <React.Fragment key={i}>{text(part)}</React.Fragment>;});
 return <div className={`record-body ${className}`}>{boundary}{body.split('\n').filter(Boolean).map((line,i)=><p key={i}>{linked(line)}</p>)}</div>;
}
