...
export const observationBody=z.string().max(100000,'正文最多 100,000 个字符').refine(v=>v.trim().length>0,'正文不能为空');
export const occurredAt=z.iso.datetime({offset:true}).nullable();
export const observationCreateInput=z.object({archive_id:z.uuid(),body:observationBody,occurred_at:occurredAt.default(null),request_id:requestId}).strict();
export const observationUpdateInput=z.object({id:z.uuid(),expected_version:expectedVersion,body:observationBody,occurred_at:occurredAt,request_id:requestId}).strict();
export type Observation={id:string;archive_id:string;author_id:string;author_name:string;created_at:string;updated_at:string;version:number;content_version:number;body:string;occurred_at:string|null;deleted:boolean;editable:boolean};
type Row=Omit<Observation,'deleted'|'editable'>&{deleted:number;archive_closed:number};
const columns='o.id,o.archive_id,o.author_id,m.name author_name,o.created_at,o.updated_at,o.version,o.content_version,o.deleted,v.body,v.occurred_at,a.closed archive_closed';
...
export class Observations{
 ...
 private present(row:Row):Observation{const {archive_closed,...o}=row;return {...o,deleted:!!row.deleted,editable:!archive_closed&&!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin')};}
 ...
 async detail(id:string){return {observation:await this.get(id)};}
 ...
 async create(input:unknown){const a=observationCreateInput.parse(input);return command(this.env,this.actor,{requestId:a.request_id,operation:'observation.create',parameters:{...a,request_id:undefined}},async()=>{
  await this.archives.get(a.archive_id,undefined,true);const id=uid(),at=now(),key=uid();
  return {result:{id,archive_id:a.archive_id,version:1,content_version:1,changed:true},statements:[
   ...
   this.stmt('INSERT INTO observation_versions(observation_id,version,body,occurred_at,editor_id,created_at) VALUES(?,1,?,?,?,?)',id,a.body,a.occurred_at,this.actor.id,at),this.touch(a.archive_id,at),
   this.archives.event(a.archive_id,'observation.created',null,{id,content_version:1},at,id),this.stmt('DELETE FROM mutation_guards WHERE id=?',key),
  ]};
 });}
 async update(input:unknown){
  ...
  return command(this.env,this.actor,{requestId:a.request_id,operation:'observation.update',parameters:{...a,request_id:undefined},requireAdmin:identity.author_id!==this.actor.id},async()=>{
   const old=await this.get(a.id);this.authorizeEdit(old);await this.archives.get(old.archive_id,undefined,true);
   if(old.version!==a.expected_version)throw new Failure(409,'VERSION_CONFLICT','观察已被更新，请重新读取后核对');
   const key=uid(),recordKey=uid(),changed=a.body!==old.body||a.occurred_at!==old.occurred_at,at=now();
   const statements=[this.openGuard(old.archive_id,key),this.recordGuard(a.id,a.expected_version,recordKey)];
   if(changed)statements.push(this.stmt('INSERT INTO observation_versions(observation_id,version,body,occurred_at,editor_id,created_at) VALUES(?,?,?,?,?,?)',a.id,old.content_version+1,a.body,a.occurred_at,this.actor.id,at),this.stmt('UPDATE observations SET version=version+1,content_version=content_version+1,updated_at=? WHERE id=?',at,a.id),this.touch(old.archive_id,at),this.archives.event(old.archive_id,'observation.edited',{id:a.id,content_version:old.content_version},{id:a.id,content_version:old.content_version+1},at,a.id));
   statements.push(this.stmt('DELETE FROM mutation_guards WHERE id IN (?,?)',key,recordKey));
   return {result:{id:a.id,archive_id:old.archive_id,version:old.version+(changed?1:0),content_version:old.content_version+(changed?1:0),changed},statements};
  });
 }
 async list(input:unknown){
  ...
  const items=rows.results.slice(0,a.limit).map(r=>this.present(r)),last=items.at(-1);return {observations:items,next_cursor:rows.results.length>a.limit&&last?`${last.updated_at}|${last.id}`:null};
 }
 async versions(input:unknown){
  ...
  const r=await this.stmt(`SELECT v.version,v.body,v.occurred_at,v.editor_id,m.name editor_name,v.created_at FROM observation_versions v JOIN members m ON m.id=v.editor_id WHERE v.observation_id=? ${a.before?'AND v.version<?':''} ORDER BY v.version DESC LIMIT ?`,a.id,...(a.before?[a.before]:[]),a.limit+1).all<Record<string,unknown>>();
  return {id:a.id,author_id:record.author_id,versions:r.results.slice(0,a.limit),next_cursor:r.results.length>a.limit?String(r.results[a.limit-1].version):null};
 }
 async timeline(input:unknown){
  ...
  const r=await this.stmt(`SELECT e.*,actor.name actor_name,o.id record_id,o.author_id,o.created_at record_created_at,o.updated_at record_updated_at,o.version record_version,o.content_version,o.deleted,m.name author_name,v.occurred_at,a.closed archive_closed,CASE WHEN o.deleted=0 AND ${latest} THEN v.body ELSE NULL END observation_body FROM archive_events e JOIN members actor ON actor.id=e.actor_id JOIN archives a ON a.id=e.archive_id LEFT JOIN observations o ON o.id=e.observation_id LEFT JOIN observation_versions v ON v.observation_id=o.id AND v.version=o.content_version LEFT JOIN members m ON m.id=o.author_id WHERE e.archive_id=? ${a.before?'AND e.seq<?':''} ORDER BY e.seq DESC LIMIT ?`,a.id,...(a.before?[a.before]:[]),a.limit+1).all<Record<string,unknown>>();
  ...
  return {events,next_cursor:r.results.length>a.limit?String(events.at(-1)?.seq):null};
 }
}
