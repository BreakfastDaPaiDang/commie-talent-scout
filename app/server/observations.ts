...
export class Observations{
 ...
 async create(input:unknown){const a=observationCreateInput.parse(input);return command(this.env,this.actor,{requestId:a.request_id,operation:'observation.create',parameters:{...a,request_id:undefined}},async()=>{
  const archive=await this.archives.get(a.archive_id,undefined,true);const id=uid(),at=now(),key=uid();
 ...
 async versions(input:unknown){
  ...
  await this.rememberReads(r.results.slice(0,a.limit).map(v=>({id:a.id,content_version:Number(v.version)})));return {id:a.id,author_id:record.author_id,versions:r.results.slice(0,a.limit),next_cursor:r.results.length>a.limit?String(r.results[a.limit-1].version):null};
 }
