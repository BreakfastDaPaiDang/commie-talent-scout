import {groupActivities} from '../shared/activity-groups.ts';
import {Reading,eventVisibility,type ReadDelivery} from './reading.ts';
import {z} from 'zod';
...
const columns='o.id,o.archive_id,o.author_id,m.name author_name,o.created_at,o.updated_at,o.version,o.content_version,o.deleted,o.deleted_at,o.deleted_by,(SELECT name FROM members WHERE id=o.deleted_by) deleted_by_name,v.body,v.occurred_at,a.closed archive_closed,a.deleted archive_deleted,m.version author_version,'+attachmentProjection('o.id','o.content_version')+' attachments_json';
...
export class Observations{
 ...
 private present(row:Row):Observation{const {archive_closed,archive_deleted,attachments_json,author_version,...o}=row;return {...o,attachments:JSON.parse(attachments_json??'[]').map((im:AttachmentInfo)=>this.images.info(im)),author_avatar_url:`${this.env.APP_ORIGIN}/avatars/members/${o.author_id}?v=${author_version}`,deleted:!!row.deleted,deletable:!archive_closed&&!archive_deleted&&!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin'),restorable:!archive_closed&&!archive_deleted&&!!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin'),editable:!archive_closed&&!archive_deleted&&!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin')};}
 async get(id:string){z.uuid().parse(id);const row=await this.stmt(`SELECT ${columns} FROM ${joins} WHERE o.id=? AND ((a.deleted=0 AND (o.deleted=0 OR o.author_id=?)) OR ?='admin')`,id,this.actor.id,this.actor.role).first<Row>();if(!row)throw new Failure(404,'NOT_FOUND','观察记录不存在或无权读取');return this.present(row);}
 ...
 async detail(id:string){const observation=await this.get(id);await this.rememberReads([observation]);const reading=(await new Reading(this.env,this.actor).observationDeliveries([observation])).get(id)??null;return {observation:{...observation,reading}};}
 ...
 async setDeleted(input:unknown,deleted:boolean){
  const a=observationStateInput.parse(input),identity=await this.get(a.id);
  const authorize=(o:Observation)=>{if(o.author_id!==this.actor.id&&this.actor.role!=='admin')throw new Failure(403,'AUTHOR_REQUIRED','仅原作者或管理员可以删除或恢复这条观察');};authorize(identity);
  return command(this.env,this.actor,{requestId:a.request_id,operation:deleted?'observation.delete':'observation.restore',parameters:a,requireAdmin:identity.author_id!==this.actor.id},async()=>{
   const old=await this.get(a.id);authorize(old);const archive=await this.archives.get(old.archive_id,undefined,true);
   if(old.version!==a.expected_version)throw new Failure(409,'VERSION_CONFLICT','观察已被更新，请重新读取后核对');
   const changed=old.deleted!==deleted,at=now(),key=uid(),recordKey=uid();
   const statements=[this.openGuard(old.archive_id,key),this.stmt('INSERT INTO mutation_guards VALUES(?,CASE WHEN EXISTS(SELECT 1 FROM observations WHERE id=? AND version=? AND deleted=?) THEN 1 ELSE 0 END)',recordKey,a.id,a.expected_version,old.deleted?1:0)];
   if(changed)statements.push(this.stmt('UPDATE observations SET deleted=?,deleted_at=?,deleted_by=?,updated_at=?,version=version+1 WHERE id=?',deleted?1:0,deleted?at:null,deleted?this.actor.id:null,at,a.id),this.touch(old.archive_id,at),this.archives.event(old.archive_id,deleted?'observation.deleted':'observation.restored',{id:a.id,deleted:old.deleted},{id:a.id,deleted},at,a.id));
   statements.push(this.stmt('DELETE FROM mutation_guards WHERE id IN (?,?)',key,recordKey));
   return {result:{id:a.id,archive_id:old.archive_id,archive_url:this.archives.url(archive),version:old.version+(changed?1:0),content_version:old.content_version,deleted,changed},statements};
  });
 }
 async list(input:unknown){
  const a=z.object({archive_id:z.uuid(),deleted:z.union([z.boolean(),z.enum(['true','false']).transform(v=>v==='true')]).default(false),before:z.string().max(150).optional(),limit:z.coerce.number().int().min(1).max(50).default(20)}).parse(input);await this.archives.get(a.archive_id);
  const where=['o.archive_id=?','o.deleted=?'],args:unknown[]=[a.archive_id,a.deleted?1:0];if(a.deleted){where.push("(o.author_id=? OR ?='admin')");args.push(this.actor.id,this.actor.role);}
  ...
  const items=rows.results.slice(0,a.limit).map(r=>this.present(r)),last=items.at(-1);await this.rememberReads(items);const deliveries=await new Reading(this.env,this.actor).observationDeliveries(items);return {observations:items.map(o=>({...o,reading:deliveries.get(o.id)??null})),next_cursor:rows.results.length>a.limit&&last?`${last.updated_at}|${last.id}`:null};
 }
 ...
 async event(id:string){z.uuid().parse(id);const event=await this.stmt('SELECT archive_id FROM archive_events WHERE id=?',id).first<{archive_id:string}>();if(!event)throw new Failure(404,'NOT_FOUND','事件不存在');const result=await this.timeline({id:event.archive_id,event_id:id});if(!result.events.length)throw new Failure(404,'NOT_FOUND','事件不存在或当前不可见');return {event:result.events[0]};}
 async timeline(input:unknown){
  const a=z.object({id:z.uuid(),event_id:z.uuid().optional(),before:z.coerce.number().int().positive().optional(),limit:z.coerce.number().int().min(1).max(50).default(20)}).parse(input);await this.archives.get(a.id);const visibility=eventVisibility(this.actor);
  const latest="e.seq=(SELECT max(t.seq) FROM archive_events t WHERE t.observation_id=e.observation_id)";
  const r=await this.stmt(`SELECT e.*,EXISTS(SELECT 1 FROM event_read_marks rm WHERE rm.member_id=? AND rm.event_id=e.id) is_read,actor.name actor_name,o.id record_id,o.author_id,o.created_at record_created_at,o.updated_at record_updated_at,o.version record_version,o.content_version,o.deleted,m.name author_name,m.version author_version,v.occurred_at,a.closed archive_closed,a.deleted archive_deleted,${attachmentProjection('o.id','o.content_version')} attachments_json,CASE WHEN o.deleted=0 AND ${latest} THEN v.body ELSE NULL END observation_body FROM archive_events e JOIN members actor ON actor.id=e.actor_id JOIN archives a ON a.id=e.archive_id LEFT JOIN observations o ON o.id=e.observation_id LEFT JOIN observation_versions v ON v.observation_id=o.id AND v.version=o.content_version LEFT JOIN members m ON m.id=o.author_id WHERE e.archive_id=? AND ${visibility.sql} ${a.event_id?'AND e.id=?':''} ${a.before?'AND e.seq<?':''} ORDER BY e.seq DESC LIMIT ?`,this.actor.id,a.id,...visibility.args,...(a.event_id?[a.event_id]:[]),...(a.before?[a.before]:[]),a.limit+1).all<Record<string,unknown>>();
  const events=r.results.slice(0,a.limit).map(e=>({id:e.id,is_read:!!e.is_read,seq:e.seq,archive_id:e.archive_id,actor_id:e.actor_id,actor_name:e.actor_name,source:e.source,kind:e.kind,created_at:e.created_at,observation_id:e.observation_id,before:e.before_json?JSON.parse(String(e.before_json)):null,after:e.after_json?JSON.parse(String(e.after_json)):null,
   observation:e.observation_body!==null?this.present({id:String(e.record_id),archive_id:a.id,author_id:String(e.author_id),author_name:String(e.author_name),created_at:String(e.record_created_at),updated_at:String(e.record_updated_at),version:Number(e.record_version),content_version:Number(e.content_version),deleted:0,deleted_at:null,deleted_by:null,deleted_by_name:null,body:String(e.observation_body),occurred_at:e.occurred_at as string|null,archive_closed:Number(e.archive_closed),archive_deleted:Number(e.archive_deleted),author_version:Number(e.author_version),attachments_json:String(e.attachments_json??'[]')}):null,
  }));
  const state=new TagState(this.env,this.actor),visible=[];for(const e of events){if(e.kind==='archive.tags_changed'){e.before=await state.filterEvent(e.before);e.after=await state.filterEvent(e.after);if(!e.before.length&&!e.after.length)continue;}visible.push(e);}await this.rememberReads(visible.flatMap(e=>e.observation?[e.observation]:[]));const deliveries=await new Reading(this.env,this.actor).deliver(visible.map(e=>({...e,id:String(e.id)})));return {activity_groups:groupActivities(visible.map(e=>({...e,id:String(e.id),archive_id:String(e.archive_id),actor_id:String(e.actor_id),created_at:String(e.created_at),kind:String(e.kind)}))).map(g=>({id:g.id,event_ids:g.events.map(e=>e.id),started_at:g.started_at,ended_at:g.ended_at})),events:visible.map(e=>({...e,reading:deliveries.get(String(e.id))??null,observation:e.observation?{...e.observation,reading:deliveries.get(String(e.id))??null}:null})),next_cursor:r.results.length>a.limit?String(events.at(-1)?.seq):null};
 }
}
