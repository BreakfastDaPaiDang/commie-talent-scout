...
type Row=Omit<Observation,'deleted'|'editable'|'deletable'|'restorable'|'attachments'|'author_avatar_url'>&{deleted:number;archive_closed:number;attachments_json:string;author_version:number};
const columns='o.id,o.archive_id,o.author_id,m.name author_name,o.created_at,o.updated_at,o.version,o.content_version,o.deleted,o.deleted_at,o.deleted_by,(SELECT name FROM members WHERE id=o.deleted_by) deleted_by_name,v.body,v.occurred_at,a.closed archive_closed,m.version author_version,'+attachmentProjection('o.id','o.content_version')+' attachments_json';
...
export class Observations{
 ...
 private present(row:Row):Observation{const {archive_closed,attachments_json,author_version,...o}=row;return {...o,attachments:JSON.parse(attachments_json??'[]').map((im:AttachmentInfo)=>this.images.info(im)),author_avatar_url:`${this.env.APP_ORIGIN}/avatars/members/${o.author_id}?v=${author_version}`,deleted:!!row.deleted,deletable:!archive_closed&&!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin'),restorable:!archive_closed&&!!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin'),editable:!archive_closed&&!row.deleted&&(row.author_id===this.actor.id||this.actor.role==='admin')};}
