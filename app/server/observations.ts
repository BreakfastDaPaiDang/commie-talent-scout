...
export class Observations{
 ...
 async timeline(input:unknown){
  ...
  const state=new TagState(this.env,this.actor),visible=[];for(const e of events){if(e.kind==='archive.tags_changed'){e.before=await state.filterEvent(e.before);e.after=await state.filterEvent(e.after);if(!e.before.length&&!e.after.length)continue;}visible.push(e);}await this.rememberReads(visible.flatMap(e=>e.observation?[e.observation]:[]));const deliveries=await new Reading(this.env,this.actor).deliver(visible.map(e=>({...e,id:String(e.id)})));return {events:visible.map(e=>({...e,reading:deliveries.get(String(e.id))??null,observation:e.observation?{...e.observation,reading:deliveries.get(String(e.id))??null}:null})),next_cursor:r.results.length>a.limit?String(events.at(-1)?.seq):null};
 }
}
