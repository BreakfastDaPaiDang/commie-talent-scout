...
export const imageGuides=[
 ...
 '上传后 get_upload_status 核实 ready，再将 attachment.id 作为 create_observation/update_observation 的 attachment_ids。准备或上传不发布记录。上传结果不明先查状态；ready 不再 PUT，pending 且未过期可用原票据重试，expired 重新准备。准备响应丢失可重新准备，不会重复发布。发布结果不明仍使用原 request_id 查询。',
