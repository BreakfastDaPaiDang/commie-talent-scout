...
export class Drafts{
 ...
 async list(){const rows=await this.stmt("SELECT archive_id,updated_at FROM observation_drafts d WHERE member_id=? AND updated_at>=? AND EXISTS(SELECT 1 FROM archives ar WHERE ar.id=d.archive_id AND ar.deleted=0) AND (length(trim(body))>0 OR EXISTS(SELECT 1 FROM draft_attachments a WHERE a.member_id=d.member_id AND a.archive_id=d.archive_id))",this.actor.id,cutoff()).all();return {drafts:rows.results};}
 ...
 async save(value:unknown){
  const a=input.parse(value),archives=new Archives(this.env,this.actor,'web'),archive=await archives.get(a.archive_id);if(archive.deleted)throw new Failure(409,'ARCHIVE_DELETED','档案已删除，请先恢复档案');
  return command(this.env,this.actor,{requestId:a.request_id,operation:'draft.save',parameters:{...a,request_id:undefined}},async()=>{
   ...
   const statements=[archives.guard(a.archive_id,archive.version,key+':archive',false),this.stmt(`INSERT INTO mutation_guards VALUES(?,CASE WHEN ${condition} THEN 1 ELSE 0 END)`,key,this.actor.id,a.archive_id,a.expected_version===0?cutoff():a.expected_version)];
   ...
   statements.push(this.stmt('DELETE FROM mutation_guards WHERE id IN (?,?)',key,key+':archive'));
