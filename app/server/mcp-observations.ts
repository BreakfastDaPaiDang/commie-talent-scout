...
export const observationGuides=[
 ...
 '当前仅开放文字观察，材料整理与标签已开放，见 compression/tags 指南；图片尚未交付。',
];
export function registerObservationTools(server:McpServer,env:Env,actor:Actor,reply:McpReply){
 ...
 server.registerTool('create_observation',{description:'按授权在确认的开启档案下发布文字观察。保留具体事件、材料时间、来源、归属与不确定性；occurred_at 仅在明确时填写，不把提交时间冒充发生时间。非空正文，原 ID 重试不重复发布。',annotations:write,inputSchema:observationCreateInput.extend({task_id:taskIdSchema}),outputSchema:version},a=>reply(()=>service.create(businessInput(a)),summary));
 ...
 server.registerTool('list_observations',{description:'分页读取指定档案的当前观察正文，按最近内容修改排序；忽略已删除记录。只读取用户任务需要的范围，以 next_cursor 判断是否完整。',annotations:read,inputSchema:{task_id:taskIdSchema,archive_id:z.uuid(),before:z.string().optional(),limit:z.number().int().min(1).max(50).default(20)},outputSchema:z.object({observations:z.array(record),next_cursor:z.string().nullable()})},a=>reply(()=>service.list(a),r=>({count:(r.observations as unknown[]).length})));
