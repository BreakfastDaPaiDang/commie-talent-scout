...
export function registerObservationTools(server:McpServer,env:Env,actor:Actor,reply:McpReply){
 ...
 const summary=(r:Record<string,unknown>)=>({id:r.id,archive_id:r.archive_id,version:r.version,content_version:r.content_version,changed:r.changed});
 ...
 server.registerTool('update_observation',{description:'原作者或管理员提交已获用户确认的观察改稿；默认先展示拟修改正文，用户只是要求润色/精简时不得直接保存。先读取当前版本并完整提供正文与发生时间。保留旧版本和原作者；无变化不生成历史，不能借编辑删除原始归属。',annotations:write,inputSchema:observationUpdateInput.extend({task_id:taskIdSchema}),outputSchema:version},a=>reply(()=>service.update(businessInput(a)),summary));
