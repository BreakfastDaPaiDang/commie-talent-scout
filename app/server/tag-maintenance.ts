...
type Category={id:string;type:'person'|'org';name:string;description:string;color:string;version:number;enabled:number;deleted:number;impact_version:number};
...
export class TagMaintenance {
 ...
 async definition(subject:Pick<Subject,'entity_type'|'id'>):Promise<(TagDefinition&{impact_version:number})|Category>{
  ...
  const category=await this.stmt('SELECT id,type,name,description,color,version,enabled,deleted,impact_version FROM tag_categories WHERE id=?',subject.id).first<Category>();if(!category)throw new Failure(404,'NOT_FOUND','类别不存在');return category;
 }
 ...
 assertMutable(d:{deleted:number;category_deleted?:number}){if(d.deleted||d.category_deleted)throw new Failure(409,'DEFINITION_DELETED','词条或类别已删除，请先由管理员恢复');}
 async preview(input:unknown){const a=tagPreviewInput.parse(input),before=await this.definition(a),expected:TagExpectation[]=[{entity_type:a.entity_type,id:a.id,version:before.version,impact_version:before.impact_version}];
  this.assertMutable(before);let after:Record<string,unknown>;
  if(a.entity_type==='tag'){
   ...
   this.assertMutable(category);if(category.type!==before.type)throw new Failure(400,'TAG_TYPE_MISMATCH','不能跨人物和组织词库换类');if(!category.enabled&&a.category_id!==current.category_id)throw new Failure(409,'CATEGORY_DISABLED','不能迁入停用类别');
 ...
 async apply(input:unknown){const a=tagApplyInput.parse(input);return command(this.env,this.actor,{requestId:a.request_id,operation:'tag.definition',parameters:{preview_id:a.preview_id}},async()=>{
  ...
  this.assertMutable(before);if(changed){
 ...
 async previewAvailability(input:unknown){assertAdmin(this.actor);const a=tagAvailabilityInput.parse(input),before=await this.definition(a),expected:TagExpectation[]=[{entity_type:a.entity_type,id:a.id,version:before.version,impact_version:before.impact_version}],after={...before,enabled:a.enabled?1:0,...(a.entity_type==='tag'&&a.enabled?{merged_into:null}:{})};
  this.assertMutable(before);if(a.entity_type==='tag'&&'category_id' in before){const category=await this.definition({entity_type:'category',id:before.category_id});this.assertMutable(category);expected.push({entity_type:'category',id:category.id,version:category.version,impact_version:category.impact_version});}
 ...
 async applyAvailability(input:unknown){const a=tagApplyInput.parse(input);return command(this.env,this.actor,{requestId:a.request_id,operation:'tag.availability',parameters:{preview_id:a.preview_id},requireAdmin:true},async()=>{
  ...
  this.assertMutable(before);if(changed){const after={...before,enabled:Number(change.enabled),version,...(change.entity_type==='tag'&&change.enabled?{merged_into:null}:{}),reason:change.reason};
