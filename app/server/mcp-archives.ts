...
export function registerArchiveTools(server:McpServer,env:Env,actor:Actor,reply:McpReply){
 ...
 server.registerTool('create_archive',{description:'按用户要求创建人物或组织档案。先查重并检查材料中的联系方式、QQ 和来源链接。默认视奸观察，工作状态必须先明确负责成员，不能默认调用者。名称不唯一、类型不可改，不创建登录账号。',annotations:write,inputSchema:archiveCreateInput,outputSchema:version},a=>reply(()=>service.create(a),r=>({id:r.id,version:r.version,changed:r.changed})));
 ...
 server.registerTool('search_members',{description:'成员按姓名分页查询协作成员的稳定 ID、显示名及冻结标记，供明确负责人或关联成员使用。此工具不提供账号管理权限；不得自行选择调用者，冻结成员不能新分配。',annotations:read,inputSchema:{query:z.string().max(100).default(''),before:z.string().optional(),limit:z.number().int().min(1).max(100).default(30)},outputSchema:z.object({members:z.array(record),next_cursor:z.string().nullable()})},a=>reply(()=>new Members(env,actor,'mcp').directory(a),r=>({count:(r.members as unknown[]).length})));
 server.registerTool('set_archive_state',{description:'按用户明确意图更改开启档案的状态和当前状态成员，可自由选择该类型合法状态。工作状态必须先明确负责成员；已入伙/已弃用会关闭并锁定档案，不能为完成其他操作擅自关闭。',annotations:{...write,destructiveHint:true},inputSchema:archiveStateInput,outputSchema:version},a=>reply(()=>service.setState(a),r=>({id:r.id,version:r.version,status:r.status,changed:r.changed})));
 server.registerTool('reopen_archive',{description:'只在用户明确要求继续维护时重新开启已关闭档案，同时选择合法开启状态及负责成员。先 get_archive 核对版本、last_open_status 与历史关联；工作状态负责人不明时先询问。普通资料写入不能替代此操作。',annotations:write,inputSchema:archiveStateInput,outputSchema:version},a=>reply(()=>service.reopen(a),r=>({id:r.id,version:r.version,status:r.status,changed:r.changed})));
}
