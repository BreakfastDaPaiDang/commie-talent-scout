...
export function registerArchiveTools(server:McpServer,env:Env,actor:Actor,reply:McpReply){
 ...
 server.registerTool('list_archives',{description:'按人物 person 或组织 org 查询档案，搜索名称、联系方式或未删除观察正文，可组合 scope/status/member_id/closed 筛选，按最近内容活动分页。创建前用于查重；同名不证明同一对象。',annotations:read,inputSchema:archiveListInput.extend({task_id:taskIdSchema}),outputSchema:z.object({archives:z.array(record),next_cursor:z.string().nullable()})},a=>reply(()=>service.list(a),r=>({count:(r.archives as unknown[]).length})));
