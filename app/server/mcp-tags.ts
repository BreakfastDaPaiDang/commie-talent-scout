import {TagDeletion,tagDeletionInput} from './tag-deletion.ts';
import {advisorRule,proposalRule,approvalRule,draftRule,compressionRule} from './mcp-consent.ts';
import {McpServer} from '@modelcontextprotocol/server';
import {z} from 'zod';
import {Tags,tagBatchInput,tagCreateInput,categoryCreateInput,tagListInput,categoryListInput} from './tags.ts';
...
export const tagToolNames=['list_tag_categories','list_tags','create_tag_category','create_tag','get_archive_tags','update_archive_tags','get_tag_definition','list_tag_bindings','preview_tag_definition','apply_tag_definition','preview_tag_migration','apply_tag_migration','preview_tag_availability','apply_tag_availability','preview_tag_deletion','apply_tag_deletion','record_task_context'];
for(const [tool,fields] of Object.entries({preview_tag_deletion:['entity_type','id','deleted','reason'],apply_tag_deletion:['preview_id','request_id'],preview_tag_migration:['change'],apply_tag_migration:['preview_id','request_id'],preview_tag_availability:['entity_type','id','enabled','reason'],apply_tag_availability:['preview_id','request_id']}))registerJournalFields(tool,fields);
...
for(const [tool,fields] of Object.entries({list_tag_categories:['type','include_disabled','include_deleted'],list_tags:['type','query','category_id','include_disabled','include_deleted','before','limit'],create_tag_category:['type','name','description','color','request_id'],create_tag:['category_id','name','description','request_id'],get_archive_tags:['archive_id'],update_archive_tags:['archive_id','expected_version','changes','focus','request_id']}))registerJournalFields(tool,fields);
export const tagGuides=[
 ...
 'list_tag_categories/list_tags 默认只返回可用词库，日常整理不要请求 include_disabled/include_deleted；仅在明确维护或追溯时扩大范围。管理员清理废弃词义用 preview_tag_deletion → apply_tag_deletion，不以停用冒充删除。删除保留明确历史引用；恢复后为停用，同名冲突不能覆盖。已获授权的同一清理范围不重复询问。',
 ...
 'get_tag_definition 查看词义、可见影响与定义历史；list_tag_bindings 分页读取开启/关闭档案及当前可见依据。更名、澄清或换类先 preview_tag_definition，核对差异与影响后 apply_tag_definition；先给用户看具体变化及影响，确认后提交；已确认同一方案不重复询问。预览失效重新核对，不循环盲试。',
...
export const compressionGuides=[
 ...
 '用户确认执行后，record_task_context 一次提供 purpose，可选原话 original_request、Agent 摘要 agent_summary、材料种类和本次源材料/引用，返回服务生成 task_id，后续调用复用。原话未知就省略，不把摘要当原话；不要求传无关聊天或内部推理。不提供任务上下文仍可正常办理。',
 '常用顺序：确认对象 → 读取旧材料/词库 → 展示精简草稿与标签建议 → 等待用户确认或修改 → 仅执行确认部分（适用时记录任务上下文）→ 核对实际结果。每次操作用独立 request_id；重试保留原 ID 和参数。多步不是一个事务，失败时保留已完成和剩余清单。',
...
export function registerTagTools(server:McpServer,env:Env,actor:Actor,reply:McpReply){
 ...
 server.registerTool('list_tag_categories',{description:'默认仅列可用类别；include_disabled 显式包含停用项，include_deleted 仅管理员列已删除项。返回人物/组织类别、描述、颜色与版本。类别名和标签名分开，优先复用。',annotations:read,inputSchema:categoryListInput.extend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>service.categories(businessInput(a))));
 server.registerTool('list_tags',{description:'默认只搜索可用词义；include_disabled 包含停用项，include_deleted 仅管理员列已删除词条及已删除类别下词条。按人物/组织、类别及名称/描述搜索词库，返回标签通用描述、category_description 类别边界、版本和当前成员可见绑定数。先读描述再复用，next_cursor 非空须按需翻页。',annotations:read,inputSchema:tagListInput.extend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>service.list(businessInput(a)),r=>({count:(r.tags as unknown[]).length})));
 ...
 const deletion=new TagDeletion(env,actor,'mcp');
 server.registerTool('preview_tag_deletion',{description:'仅管理员：预览删除或恢复词条/类别。deleted=true 从词库移除并隐藏类别下词条，保留已有绑定和历史引用；false 恢复为停用，同名冲突拒绝。先报告具体对象与影响。',annotations:read,inputSchema:tagDeletionInput.extend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>deletion.preview(businessInput(a))));
 server.registerTool('apply_tag_deletion',{description:'仅管理员：提交已经授权的删除/恢复预览。版本或绑定变化需重新预览；原 request_id 重试不会重复删除。删除释放名称，旧引用保留；恢复后仍需明确启用。',annotations:{...write,destructiveHint:true},inputSchema:tagApplyInput.extend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>deletion.apply(businessInput(a)),summary));
 server.registerTool('create_tag_category',{description:'仅在用户确认具体新类别建议后创建，不因整理请求自动创建。人物/组织独立，同库规范化重名返回已有 ID，不覆盖定义。',annotations:write,inputSchema:categoryCreateInput.extend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>service.createCategory(businessInput(a)),summary));
 server.registerTool('create_tag',{description:'先建议新词条名称、类别、通用描述及必要性，用户确认后才创建；整理请求不自动授权新建。category_id 与无冒号的名称分开提供；重名复用，不自动绑定。禁止把具体档案材料写入公共定义。',annotations:write,inputSchema:tagCreateInput.extend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>service.create(businessInput(a)),summary));
 ...
 server.registerTool('update_archive_tags',{description:'先向用户列出标签增减/依据更新建议及理由，确认该范围后再批量执行，一批一个事件；仅确认观察不授权标签变更。先读旧依据和实际记录版本；Agent 每个新增/依据更新须有来源，重复添加不覆盖。新材料未提及不得据此移除。重点最多三项且不猜测。',annotations:write,inputSchema:tagBatchInput.safeExtend({task_id:taskIdSchema}),outputSchema:output},a=>reply(()=>service.batch(businessInput(a)),summary));
 server.registerTool('record_task_context',{description:'用户确认执行材料整理方案后一次轻量记录任务目的、可选用户原话/Agent 摘要、源材料及引用，返回 task_id 供后续调用关联。缺原话标未知，不采集无关会话；不强制开始/结束才能办理业务。提交内容私有留存 30 天用于排错和改进。',annotations:write,inputSchema:taskContextInput,outputSchema:output},a=>reply(()=>recordTaskContext(env,actor,a),summary));
}
