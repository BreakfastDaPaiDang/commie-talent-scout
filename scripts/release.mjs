...
try{const result=await releaseStages({
 inspect:async()=>{const response=await cf(pointer,{raw:true,allowMissing:true});return {version:await currentVersion(),record:response?await response.json():null};},
 ...
 verifyRollback:async version=>{if(await currentVersion()!==version)throw new Error('Active code version does not match rollback target');await smoke(environment,{compareAssets:false});},
