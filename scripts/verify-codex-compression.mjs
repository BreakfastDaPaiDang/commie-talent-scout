...
try{
 ...
 const primary=await run('primary',`请使用已经接好的康米巨星猎头系统，把下面聊天整理进人物「${name}」的档案。这是本次验收专用的虚构材料，本次明确免预览直接执行：在这份档案内保存精简观察，并增减/创建有依据的标签，无需先给草稿。\n\n${chat}`);
 ...
 const recovered=await run('lost-response',`请用已连接的系统给「虚构响应丢失恢复 ${suffix}」补充观察：2026-09-08，本人明确表示本月愿意按项目参与音频后期，具体排期另行约定。把有依据且适用的标签一并整理好。这是虚构验收资料；本次明确免预览直接发布上述观察并整理有依据的标签，无需先给草稿。`);
