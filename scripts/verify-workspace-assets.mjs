...
const base=process.argv.includes('--remote')?'https://scout-staging.dapaidang.org':'http://127.0.0.1:8790';
...
assert.equal(await fetch(base+cssPath).then(r=>r.text()),css);
...
try{
 ...
 assert.equal(style.backgroundColor,'rgb(255, 255, 255)');assert.equal(style.borderRadius,'0px');
