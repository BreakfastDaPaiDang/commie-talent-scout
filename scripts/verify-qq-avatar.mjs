...
try{
 ...
 for(const width of [1440,390]){await page.setViewportSize({width,height:1000});await page.goto(base+'/admin/members');const avatar=page.locator('img[src*="/avatars/members/'+member.id+'"]');await avatar.first().waitFor();await avatar.first().evaluate(async image=>{await image.decode();if(image.naturalWidth!==640||image.naturalHeight!==640)throw new Error('QQ image did not decode at its original dimensions');});await page.screenshot({path:out+'/'+width+'.png'});}
