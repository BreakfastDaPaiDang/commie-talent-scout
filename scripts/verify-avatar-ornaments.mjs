...
try{
 ...
 for(let i=0;i<ids.length;i++){
  ...
  const decoded=await page.evaluate(async id=>{const image=new Image();image.src=`/art/avatar-ornaments/${id}.webp`;await image.decode();const canvas=document.createElement('canvas');canvas.width=image.width;canvas.height=image.height;const ctx=canvas.getContext('2d');ctx.drawImage(image,0,0);const data=ctx.getImageData(0,0,canvas.width,canvas.height).data;let clear=0,ink=0;for(let i=3;i<data.length;i+=4){if(data[i]===0)clear++;if(data[i]>128)ink++;}return {clear,ink};},ids[i]);assert.ok(decoded.clear>1000&&decoded.ink>1000);
  const served=await page.evaluate(async id=>{const r=await fetch(`/art/avatar-ornaments/${id}.webp`),bytes=await r.arrayBuffer();return {type:r.headers.get('content-type'),hash:[...new Uint8Array(await crypto.subtle.digest('SHA-256',bytes))].map(b=>b.toString(16).padStart(2,'0')).join('')};},ids[i]);assert.match(served.type,/^image\/webp/);assert.equal(served.hash,createHash('sha256').update(readFileSync(`public/art/avatar-ornaments/${ids[i]}.webp`)).digest('hex'));
  ...
  for(const width of [1440,1100,1024,768,390]){
   ...
   assert.ok(!measured.shown||(width>=1100&&measured.header>=760));assert.equal(measured.overflow,false);
  }
 ...
 checks.push('all six selections load real transparent WebP bytes against staging and keep the original colored avatar','editing, composing and resizing retain the draw; 5 narrow widths and expanded reading obey available header space');
