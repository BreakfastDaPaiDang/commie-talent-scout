...
export async function smoke(environment,{compareAssets=true,expectedVersion=JSON.parse(readFileSync('package.json','utf8')).version}={}){
 ...
 const health=await request('/api/health');assert.equal(health.status,200);assert.deepEqual(await health.json(),{ok:true,version:expectedVersion,environment});
