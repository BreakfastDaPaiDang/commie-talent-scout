...
QQ 头像沿用 makeshift-dev 的本域代理与过期缓存思路；在本系统通过成员/档案标识鉴权，使用私有缓存。无 QQ、获取失败、头像来源变更及关闭档案的行为见[设计说明](./design/README.md)。头像已纳入首版：人物 QQ 自动获取、组织私有上传；分别设计占位图，不能用首字。
...
Agent 接入主页面只承担使用说明与复制提示词。机器配置和业务规则进入复制内容，个人凭证生命周期在个人账号连接管理中完成；这不改变 MCP 的每请求认证或权限边界。
