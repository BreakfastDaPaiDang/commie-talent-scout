...
- 用户已提供账单截图，确认 Workers Paid 和 R2 Paid 均处于活动状态，并接受本项目每月 $10 的云费用预算目标。沿用现有订阅，不重复开通；预估超过预算时再确认。当前 API 账单读取范围不影响该用户确认。
...
未读以个人事件标记为基础，排除当前成员自身动作，并从成员创建后的事件开始。网页采用 [ADR 0012](./adr/0012-mail-style-archive-reading.md) 的邮件式阅读：详情响应固定本次打开的事件范围，前台成功打开后一次确认，后来的新事件不被列表或详情自动重读清除。后台、失败、隐藏详情及权限拒绝不确认；写入幂等且不改活动时间。MCP 继续使用 ADR 0006 的逐事件内容票据，元数据读取不确认。两者共享权限过滤及 `event_read_marks`。系统事件仍轻量展示，记录内容为主。
...
正式入口采用用户已确认的 `scout.dapaidang.org`。Worker 静态资源配置明确区分页面与 `/api`、`/mcp`、`/images`、`/uploads`，API 错误不回退为 SPA HTML。
