...
本地套件重新执行，16 项通过；其中删除用例已扩展，验证非作者普通成员不能通过 HTTP/MCP 读取删除正文、版本、附件或旧图片地址，原作者/管理员可读，恢复后其他成员重新可读。[结果](./validation/local-v1.2-deleted-visibility.json)。这是本地实测；此前云端 16 项通过的证据不覆盖这项新规则，临时云资源已清理，正式实现须云端复验。新状态必选绑定和头像目前由原型/方案覆盖，不能称为云端已验证。
